...
export class SimpleDocumentService {
  ...
  async convertFile(inputPath: string, outputPathOrOptions: string | Partial<ConversionOptions>, options?: Partial<ConversionOptions>): Promise<ConversionResult> {
    ...
    // Calculate path depth for image path correction if output path is provided
    let pathDepth = actualOptions.pathDepth;
    if (!pathDepth && outputPath) {
      try {
        const { relative, dirname, sep } = await import('path');
        const outputDir = actualOptions.outputDir || dirname(outputPath);
        const relativeOutputPath = relative(outputDir, outputPath);
        pathDepth = dirname(relativeOutputPath).split(sep).filter(part => part && part !== '.').length;
      } catch (error) {
        // Fallback - no path depth correction
        pathDepth = 0;
      }
    }
    ...
    const conversionOptions: ConversionOptions = {
      ...
      inputPath: inputPath,
      pathDepth: pathDepth || actualOptions.pathDepth,
    ...
    if (outputPath) {
      ...
      // Write glossary file if it was generated
      if (result.glossaryContent && format === 'asciidoc' && actualOptions.asciidocOptions?.glossaryOptions?.includeGlossary) {
        const outputDir = actualOptions.outputDir || dirname(outputPath);
        const glossaryFormat = actualOptions.asciidocOptions.glossaryOptions.glossaryFormat || 'separate';
        let glossaryPath: string;
        
        if (glossaryFormat === 'separate') {
          glossaryPath = `${outputDir}/glossary.adoc`;
        } else if (glossaryFormat === 'book-appendix') {
          await this.ensureDirectoryExists(`${outputDir}/appendices`);
          glossaryPath = `${outputDir}/appendices/glossary.adoc`;
        } else {
          await this.ensureDirectoryExists(`${outputDir}/includes`);
          glossaryPath = `${outputDir}/includes/glossary.adoc`;
        }
        
        await writeFile(glossaryPath, result.glossaryContent, 'utf8');
      }
    }
