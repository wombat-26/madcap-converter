  ...
  // Broken List Structure
  {
    ...
    validate: (content: string): ValidationIssue[] => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Enhanced to properly distinguish list items from emphasis markers
        if (line.match(/^\s*[.*]+\s+/) && i > 0) {
          const currentIndent = line.match(/^(\s*)/)?.[1].length || 0;
          const currentMarkerType = line.match(/^(\s*)([.*]+)/)?.[2][0];
          
          // Skip if this looks like emphasis text instead of a list item
          if (currentMarkerType === '*' && line.match(/^\s*\*[^*\s]+.*\*\s*$/)) {
            continue; // This is likely emphasis text (*text*), not a list item
          }
          ...
          for (let j = i - 1; j >= 0; j--) {
            ...
            // Skip if previous line looks like emphasis text instead of a list item
            if (prevMarkerType === '*' && prevLine.match(/^\s*\*[^*\s]+.*\*\s*$/)) {
              continue; // This is likely emphasis text (*text*), not a list item
            }
